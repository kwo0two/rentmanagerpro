...
interface TenantsByBuildingTableProps<TData, TValue> {
  columns: ColumnDef<TData, TValue>[];
  buildingId: string;
}

export function TenantsByBuildingTable<TData extends TenantLeaseInfo, TValue>({
  columns,
  buildingId,
}: TenantsByBuildingTableProps<TData, TValue>) {
        ...
        try {
            ...
            const leaseDataWithDetails = leases.map(lease => {
                ...
                while (currentDate <= today && currentDate <= leaseEndDate) {
                    totalDue += rentAmount; // Simplified for this table
    ...
    <div>
      ...
      {showNoTenantsMessage ? (
        <div className="flex flex-col items-center justify-center rounded-lg border border-dashed shadow-sm p-8 text-center h-[400px]">
            ...
            <h2 className="mt-6 text-xl font-semibold">이 건물에 등록된 임차인이 없습니다.</h2>
